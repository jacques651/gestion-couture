...
import {
  ...
  IconCheck, IconCalendar, IconClock, IconCalendarEvent,
  IconInfoCircle
...
const VentesManager: React.FC = () => {
  ...
  const [rdvOpen, setRdvOpen] = useState(true);
  ...
  const handleAnnulerVente = async (venteId: number, codeVente: string) => {
    ...
    try {
      ...
      await journaliserAction({
        ...
        details: `Annulation vente : ${codeVente}`
      });
      notifications.show({
        title: 'Succès',
        message: `Vente ${codeVente} annulée`,
        color: 'green'
      });
      await loadVentes();
    } catch (err: any) {
  ...
  const handleDeleteVente = async () => {
    ...
    try {
      ...
      await journaliserAction({
        ...
        details: `Suppression vente : ${deleteVenteCode}`
      });
      notifications.show({
        title: 'Succès',
        message: `Vente ${deleteVenteCode} supprimée`,
        color: 'green'
      });
  ...
  const loadFormData = async () => {
    try {
      ...
      setModeles(modelesData.filter((m: any) => m.est_actif === 1));
  ...
  const handleEditVente = async (vente: Vente) => {
    try {
      ...
      setEditVenteData({
        ...
        lignes: detailsData || [],
        rendezvous: rendezvousData  // Ajouter les infos rendez-vous
      });
  ...
  const handleSaveEditVente = async () => {
    ...
    setEditLoading(true);
    try {
      const newTotal = (editVenteData.lignes || []).reduce((s: number, l: LigneEdit) => s + (l.quantite * l.prix_unitaire), 0);

      await updateVente(editVenteData.id, {
        ...
        rendezvous: editVenteData.type_vente === 'commande' && editVenteData.rendezvous?.date_rendezvous ? editVenteData.rendezvous : null
      });
      ...
      await journaliserAction({
        ...
        details: `Modification vente : ${editVenteData.code_vente} - ${newTotal.toLocaleString()} FCFA`
      });
  ...
  const handleAjouterAuPanier = () => {
    if (!selectedArticle || quantiteCmd > selectedArticle.quantite_stock) {
      notifications.show({ title: 'Erreur', message: `Stock insuffisant (max: ${selectedArticle?.quantite_stock})`, color: 'red' });
      return;
    }
    const mm = modeles?.find(m => m.designation === selectedArticle.modele);
    const tt = tailles?.find(t => t.libelle === selectedArticle.taille);
    let des = selectedArticle.modele;
    if (mm) des += ` (${mm.categorie})`;
    des += ` - ${tt?.code_taille || selectedArticle.taille} - ${selectedArticle.couleur}`;
    if (selectedArticle.texture) des += ` - (${selectedArticle.texture})`;
    const ex = panier.findIndex(item => item.produitId === selectedArticle.id && item.type_produit === 'article');
    if (ex >= 0) {
      const up = [...panier];
      const nq = up[ex].quantite + quantiteCmd;
      if (nq > selectedArticle.quantite_stock) {
        notifications.show({ title: 'Erreur', message: 'Stock insuffisant', color: 'red' });
        return;
      }
      up[ex].quantite = nq;
      up[ex].total = up[ex].prixUnitaire * nq;
      setPanier(up);
      notifications.show({ title: 'Mis à jour', message: `${des} : ${nq} x ${up[ex].prixUnitaire.toLocaleString()} FCFA`, color: 'green' });
    } else {
      setPanier([...panier, { id: `${Date.now()}-${Math.random()}`, produitId: selectedArticle.id, designation: des, taille: tt?.code_taille || selectedArticle.taille, couleur: selectedArticle.couleur, quantite: quantiteCmd, prixUnitaire: selectedArticle.prix_vente, total: selectedArticle.prix_vente * quantiteCmd, type_produit: 'article' }]);
      notifications.show({ title: 'Ajouté', message: `${des} x${quantiteCmd}`, color: 'green' });
    }
  ...
  const handleAjouterMatiereAuPanier = (matiere: Matiere) => {
    const ex = panier.findIndex(item => item.produitId === matiere.id && item.type_produit === 'matiere');
    if (ex >= 0) {
      if (panier[ex].quantite + 1 > matiere.stock_actuel) {
        notifications.show({ title: 'Erreur', message: 'Stock insuffisant', color: 'red' });
        return;
      }
      const up = [...panier];
      up[ex].quantite += 1;
      up[ex].total = up[ex].prixUnitaire * up[ex].quantite;
      setPanier(up);
      notifications.show({ title: 'Mis à jour', message: `${matiere.designation} : ${up[ex].quantite} unité(s)`, color: 'green' });
    } else {
      setPanier([...panier, { id: `${Date.now()}-${Math.random()}`, produitId: matiere.id, designation: matiere.designation, quantite: 1, prixUnitaire: matiere.prix_vente, total: matiere.prix_vente, type_produit: 'matiere' }]);
      notifications.show({ title: 'Ajouté', message: `${matiere.designation} x1`, color: 'green' });
    }
  };
  ...
  const handleGenererFacture = () => {
    if (panier.length === 0) {
      notifications.show({ title: 'Erreur', message: 'Ajoutez des articles à la commande', color: 'red' });
      return;
    }

    setFactureData({
      id: venteId, // Ajoutez l'ID si disponible
      code_vente: codeVente,
      date_vente: dateCommande,
      client: {
        nom_prenom: clientNom || (clientId ? clients.find(c => c.telephone_id === clientId)?.nom_prenom : 'Client'),
        telephone_id: clientTelephone || clientId || ''
      },
      lignes: panier.map(item => ({  // Assurez-vous que lignes est bien passé
        designation: item.designation,
        quantite: item.quantite,
        prix_unitaire: item.prixUnitaire,
        total: item.total
      })),
      total_general: totalPanier,
      avance: 0,
      reste: totalPanier,
      numero: codeVente,
  ...
  const handleTerminerRendezVous = async (id: number) => {
    try {
      await terminerRendezVous(id);
      loadRendezVous();
      notifications.show({ title: 'Succès', message: 'Rendez-vous terminé', color: 'green' });
    } catch (err) {
      console.error(err);
  ...
  const handleAnnulerRendezVous = async (id: number) => {
    try {
      ...
      loadRendezVous();
      notifications.show({ title: 'Succès', message: 'Rendez-vous annulé', color: 'green' });
    } catch (err) {
      console.error(err);
      notifications.show({ title: 'Erreur', message: 'Erreur lors de la mise à jour', color: 'red' });
    }
  ...
  const loadRendezVous = async () => {
    try {
      const rows = await getRendezVous();
      setRendezVous(rows);
    } catch (err) {
      console.error(err);
    }
  ...
  if (showFacture && factureData)
    return (
      <ModalFacture
        ...
        onConfirmPaiement={(m, mode) => { submitVenteAvecPaiement(m, mode); }}
  ...
  if (showRecu && venteIdForRecu)
    return (
      <ModalRecu
        ...
        onClose={() => { setShowRecu(false); setVenteIdForRecu(null); backToList(); }}
      />
    );
    ...
    return (
      <Box p="md">
          ...
          <Stack gap="lg">
            <Card withBorder radius="lg" p="lg" style={{ background: 'linear-gradient(135deg, #1b365d 0%, #2a4a7a 100%)' }}>
              <Group justify="space-between">
                <Group gap="md">
                  <Avatar size={50} radius="md" style={{ backgroundColor: 'rgba(255,255,255,0.2)' }}><IconShoppingBag size={24} color="white" /></Avatar>
                  <Box><Title order={2} c="white">Gestion des Ventes</Title><Text c="gray.3" size="sm">Consultez l'historique et créez de nouvelles ventes</Text></Box>
                </Group>
            ...
            <Card withBorder radius="lg" shadow="sm" p="lg">
              <Group mb="md">
                <TextInput placeholder="Rechercher par code ou client..." leftSection={<IconSearch size={16} />} value={searchTerm} onChange={(e) => { setSearchTerm(e.target.value); setCurrentPage(1); }} style={{ flex: 1 }} />
                <Tooltip label="Actualiser"><ActionIcon variant="light" onClick={() => { loadVentes(); loadRendezVous(); }} size="lg"><IconRefresh size={18} /></ActionIcon></Tooltip>
              </Group>
              <LoadingOverlay visible={loading} />

              {/* Section Rendez-vous mise en valeur */}
              <Card withBorder radius="lg" shadow="sm" p="md" mb="md" style={{ backgroundColor: '#FFF8E7', borderLeft: '4px solid #1b365d' }}>
                <Group justify="space-between" mb="sm">
                  <Group gap="xs">
                    <IconCalendarEvent size={24} color="#1b365d" />
                    <Title order={5} c="#1b365d">📅 Rendez-vous à venir</Title>
                    <Badge size="lg" variant="filled" color="#1b365d">
                      {rendezVous.filter(r => r.statut === 'planifie').length} en attente
                    </Badge>
                  </Group>
                  <ActionIcon
                    variant="subtle"
                    onClick={() => setRdvOpen(!rdvOpen)}
                    size="md"
                    color="#1b365d"
                  >
                    {rdvOpen ? <IconChevronUp size={18} /> : <IconChevronDown size={18} />}
                  </ActionIcon>
                </Group>
                <Divider mb="sm" />

                {/* Au lieu de Collapse, on utilise un div avec display conditionnel */}
                <div style={{ display: rdvOpen ? 'block' : 'none' }}>
                  {rendezVous.filter(r => r.statut === 'planifie').length === 0 ? (
                    <Text ta="center" c="dimmed" py="md">Aucun rendez-vous programmé</Text>
                  ) : (
                    <ScrollArea h={300}>
                      <Table striped highlightOnHover>
                        <Table.Thead style={{ backgroundColor: '#1b365d', position: 'sticky', top: 0, zIndex: 1 }}>
                          <Table.Tr>
                            ...
                            <Table.Th style={{ color: 'white', fontSize: 12 }}>Heure</Table.Th>
                            <Table.Th style={{ color: 'white', fontSize: 12 }}>Client</Table.Th>
                            <Table.Th style={{ color: 'white', fontSize: 12 }}>Commande</Table.Th>
                            <Table.Th style={{ color: 'white', fontSize: 12 }}>Type</Table.Th>
                            <Table.Th style={{ color: 'white', fontSize: 12 }}>Statut</Table.Th>
                            <Table.Th style={{ color: 'white', fontSize: 12, textAlign: 'center' }}>Actions</Table.Th>
                          </Table.Tr>
                        </Table.Thead>
                        <Table.Tbody>
                          {rendezVous.filter(r => r.statut === 'planifie').map((r) => (
                            <Table.Tr key={r.id}>
                              <Table.Td><Text fw={500}>{new Date(r.date_rendezvous).toLocaleDateString('fr-FR')}</Text></Table.Td>
                              <Table.Td><Badge variant="light" color="blue">{r.heure_rendezvous || '-'}</Badge></Table.Td>
                              <Table.Td><Group gap="xs"><IconUser size={14} /><Text size="sm">{r.nom_prenom}</Text></Group></Table.Td>
                              <Table.Td><Badge variant="light" color="violet">{r.code_vente}</Badge></Table.Td>
                              <Table.Td>
                                <Badge color={
                                  r.type_rendezvous === 'essayage' ? 'pink' :
                                    r.type_rendezvous === 'livraison' ? 'cyan' : 'orange'
                                }>
                                  {r.type_rendezvous === 'essayage' ? '👗 Essayage' :
                                    r.type_rendezvous === 'livraison' ? '🚚 Livraison' : '📦 Retrait'}
                                </Badge>
                              </Table.Td>
                              <Table.Td>
                                <Badge color={r.statut === 'planifie' ? 'orange' : r.statut === 'termine' ? 'green' : 'red'}>
                                  {r.statut === 'planifie' ? '⏳ Planifié' : r.statut === 'termine' ? '✅ Terminé' : '❌ Annulé'}
                                </Badge>
                              </Table.Td>
                              <Table.Td>
                                <Group gap={4} justify="center">
                                  {r.statut === 'planifie' && (
                                    <>
                                      <Tooltip label="Marquer comme terminé">
                                        <ActionIcon color="green" variant="light" onClick={() => handleTerminerRendezVous(r.id)}>
                                          <IconCheck size={16} />
                                        </ActionIcon>
                                      </Tooltip>
                                      <Tooltip label="Annuler le rendez-vous">
                                        <ActionIcon color="red" variant="light" onClick={() => handleAnnulerRendezVous(r.id)}>
                                          <IconX size={16} />
                                        </ActionIcon>
                                      </Tooltip>
                                    </>
              ...
              <Table striped highlightOnHover>
                <Table.Thead style={{ backgroundColor: '#1b365d' }}>
                  <Table.Tr>
                    ...
                    <Table.Th style={{ color: 'white' }}>Total</Table.Th>
                    <Table.Th style={{ color: 'white' }}>Réglé</Table.Th>
                    <Table.Th style={{ color: 'white' }}>Reste</Table.Th>
                    <Table.Th style={{ color: 'white' }}>Statut</Table.Th>
                    <Table.Th style={{ color: 'white', textAlign: 'center' }} colSpan={2}>Actions</Table.Th>
                  </Table.Tr>
                </Table.Thead>
                <Table.Tbody>
                  {paginatedVentes.length === 0 ? (
                    <Table.Tr><Table.Td colSpan={10} style={{ textAlign: 'center' }}>Aucune vente trouvée</Table.Td></Table.Tr>
                  ) : (
                    paginatedVentes.map((vente) => {
                      const montantRestant = Math.max(0, Number(vente.montant_total || 0) - Number(vente.montant_regle || 0));
                      const isCommandeSurMesure = vente.type_vente === 'commande';
                      return (
                        <Table.Tr key={vente.id}>
                          <Table.Td><Badge variant="light" color="blue" size="md">{vente.code_vente}</Badge></Table.Td>
                          <Table.Td>
                            <Badge size="sm" variant="light" color={isCommandeSurMesure ? 'violet' : vente.type_vente === 'pret_a_porter' ? 'cyan' : 'teal'}>
                          ...
                          <Table.Td>
                            <Text c={montantRestant < 0 ? "red" : montantRestant === 0 ? "green" : "orange"} fw={600}>
                              {formatPrice(montantRestant < 0 ? 0 : montantRestant)}
                            </Text>
                          </Table.Td>
            ...
            <Stack gap="md">
              <Paper p="md" radius="md" withBorder bg="gray.0">
                <SimpleGrid cols={{ base: 2, sm: 4 }} spacing="md">
                  <Box><Text size="xs" c="dimmed">Type</Text><Badge variant="light" color={selectedVente.type_vente === 'commande' ? 'violet' : selectedVente.type_vente === 'pret_a_porter' ? 'cyan' : 'teal'}>{selectedVente.type_vente === 'commande' ? 'Sur mesure' : selectedVente.type_vente === 'pret_a_porter' ? 'Prêt-à-porter' : 'Matière'}</Badge></Box>
                  <Box><Text size="xs" c="dimmed">Client</Text><Text size="sm" fw={500}>{selectedVente.client_nom || '-'}</Text></Box>
                  <Box><Text size="xs" c="dimmed">Date</Text><Text size="sm">{new Date(selectedVente.date_vente).toLocaleDateString('fr-FR')}</Text></Box>
                  <Box><Text size="xs" c="dimmed">Paiement</Text><Text size="sm">{selectedVente.mode_paiement || '-'}</Text></Box>
                </SimpleGrid>
              </Paper>
              ...
              <Paper p="md" radius="md" withBorder>
                ...
                <Table striped highlightOnHover>
                  <Table.Thead><Table.Tr><Table.Th>Désignation</Table.Th><Table.Th ta="center">Qté</Table.Th><Table.Th ta="right">Prix unit.</Table.Th><Table.Th ta="right">Total</Table.Th></Table.Tr></Table.Thead>
                  <Table.Tbody>
                    {selectedDetails.length > 0 ? selectedDetails.map((detail: any, index: number) => (
                      <Table.Tr key={detail.id || index}>
                        <Table.Td><Text size="sm" fw={500}>{detail.designation || '-'}</Text>{detail.taille_libelle && detail.taille_libelle !== 'null' && <Text size="xs" c="dimmed">{detail.taille_libelle}</Text>}</Table.Td>
                        <Table.Td ta="center">{detail.quantite || 0}</Table.Td>
                        <Table.Td ta="right">{formatPrice(detail.prix_unitaire || 0)}</Table.Td>
                        <Table.Td ta="right"><Text fw={600}>{formatPrice(detail.total || 0)}</Text></Table.Td>
                      </Table.Tr>
                    )) : (<Table.Tr><Table.Td colSpan={4} ta="center" py="md"><Text c="dimmed">Aucun détail disponible</Text></Table.Td></Table.Tr>)}
                  </Table.Tbody>
                </Table>
                <Divider my="sm" />
                <Group justify="space-between"><Text fw={700}>Total général</Text><Text fw={700} size="lg" c="blue">{formatPrice(selectedVente.montant_total || 0)}</Text></Group>
                <Group justify="space-between" mt={4}><Text size="sm" c="dimmed">Réglé</Text><Text size="sm" c="green">{formatPrice(selectedVente.montant_regle || 0)}</Text></Group>
                <Group justify="space-between"><Text size="sm" c="dimmed">Reste</Text><Text size="sm" c={Number(selectedVente.montant_total || 0) - Number(selectedVente.montant_regle || 0) < 0 ? "red" : "orange"}>
                  {formatPrice(Math.max(0, Number(selectedVente.montant_total || 0) - Number(selectedVente.montant_regle || 0)))}
                </Text></Group>
              </Paper>

              {selectedVente.observation && (
                <Paper p="md" radius="md" withBorder bg="gray.0"><Text size="xs" c="dimmed" mb={4}>📝 Observation</Text><Text size="sm">{selectedVente.observation}</Text></Paper>
              )}

              <Group justify="flex-end" gap="sm">
                <Button variant="light" onClick={() => setDetailsModalOpen(false)}>Fermer</Button>
                <Button leftSection={<IconReceipt size={16} />} variant="light" color="grape" onClick={() => { setDetailsModalOpen(false); handleShowRecu(selectedVente); }}>Reçu</Button>
                {selectedVente.type_vente === 'commande' && (
                  <Button leftSection={<IconFileInvoice size={16} />} variant="light" color="teal" onClick={() => { setDetailsModalOpen(false); handleShowFacture(selectedVente); }}>Facture</Button>
                )}
              </Group>
            </Stack>
          )}
            ...
            <Stack gap="md">
              <Paper p="md" radius="md" withBorder>
                <SimpleGrid cols={{ base: 1, sm: 2 }} spacing="md">
                  <TextInput label="Date" type="date" value={editVenteData.date_vente?.split('T')[0] || ''} onChange={(e) => setEditVenteData({ ...editVenteData, date_vente: e.target.value })} size="sm" radius="md" />
                  <Select label="Type" data={[{ value: 'commande', label: '📝 Sur mesure' }, { value: 'pret_a_porter', label: '👕 Prêt-à-porter' }, { value: 'matiere', label: '📦 Matière' }]} value={editVenteData.type_vente} onChange={(val) => setEditVenteData({ ...editVenteData, type_vente: val })} size="sm" radius="md" />
                </SimpleGrid>
                <SimpleGrid cols={{ base: 1, sm: 2 }} spacing="md" mt="sm">
                  <TextInput label="Client" value={editVenteData.client_nom || ''} onChange={(e) => setEditVenteData({ ...editVenteData, client_nom: e.target.value })} size="sm" radius="md" />
                  <TextInput label="Téléphone" value={editVenteData.client_telephone || ''} onChange={(e) => setEditVenteData({ ...editVenteData, client_id: e.target.value })} size="sm" radius="md" />
                </SimpleGrid>
                <Textarea label="Observation" value={editVenteData.observation || ''} onChange={(e) => setEditVenteData({ ...editVenteData, observation: e.target.value })} rows={2} size="sm" radius="md" mt="sm" />
              </Paper>

              {/* Section Rendez-vous pour les commandes */}
              {editVenteData.type_vente === 'commande' && (
                <Card withBorder radius="md" p="md" style={{ backgroundColor: '#FFF8E7', borderLeft: '4px solid #1b365d' }}>
                  ...
                  <SimpleGrid cols={{ base: 1, sm: 3 }} spacing="md">
                    <TextInput label="Date du rendez-vous" type="date" placeholder="jj/mm/aaaa" leftSection={<IconCalendar size={14} />} value={editVenteData.rendezvous?.date_rendezvous || ''} onChange={(e) => setEditVenteData({ ...editVenteData, rendezvous: { ...editVenteData.rendezvous, date_rendezvous: e.target.value, statut: 'planifie' } })} size="sm" radius="md" />
                    <TextInput label="Heure" type="time" leftSection={<IconClock size={14} />} value={editVenteData.rendezvous?.heure_rendezvous || ''} onChange={(e) => setEditVenteData({ ...editVenteData, rendezvous: { ...editVenteData.rendezvous, heure_rendezvous: e.target.value } })} size="sm" radius="md" />
                    <Select label="Type de rendez-vous" data={[{ value: 'essayage', label: '👗 Essayage' }, { value: 'livraison', label: '🚚 Livraison' }, { value: 'retrait', label: '📦 Retrait' }]} value={editVenteData.rendezvous?.type_rendezvous || 'essayage'} onChange={(val) => setEditVenteData({ ...editVenteData, rendezvous: { ...editVenteData.rendezvous, type_rendezvous: val } })} size="sm" radius="md" />
                  </SimpleGrid>
                  {editVenteData.rendezvous?.statut && editVenteData.rendezvous.statut !== 'planifie' && (
                    <Alert color="blue" mt="sm">
                      <Group gap="xs"><IconInfoCircle size={14} />Ce rendez-vous est {editVenteData.rendezvous.statut === 'termine' ? 'déjà terminé' : 'annulé'}. Seuls les rendez-vous planifiés peuvent être modifiés.</Group>
                    </Alert>
                  )}
                </Card>
              )}
              ...
              <Paper p="md" radius="md" withBorder>
                <Group justify="space-between" mb="sm"><Text fw={600} size="sm">📋 Articles</Text><Button variant="light" size="compact-sm" onClick={handleAddEditLigne} leftSection={<IconPlus size={14} />}>Ajouter</Button></Group>
                <ScrollArea h={300}>
                  <Table striped highlightOnHover>
                    <Table.Thead><Table.Tr><Table.Th>Désignation</Table.Th><Table.Th w={70} ta="center">Qté</Table.Th><Table.Th w={110} ta="right">Prix unit.</Table.Th><Table.Th w={110} ta="right">Total</Table.Th><Table.Th w={40}></Table.Th></Table.Tr></Table.Thead>
                    <Table.Tbody>
                      {(editVenteData.lignes || []).map((l: LigneEdit, i: number) => (
                        <Table.Tr key={i}>
                          <Table.Td><TextInput size="xs" value={l.designation} onChange={(e) => handleEditLigneChange(i, 'designation', e.target.value)} /></Table.Td>
                          <Table.Td><NumberInput size="xs" min={1} value={l.quantite} onChange={(val) => handleEditLigneChange(i, 'quantite', val)} /></Table.Td>
                          <Table.Td><NumberInput size="xs" min={0} step={100} value={l.prix_unitaire} onChange={(val) => handleEditLigneChange(i, 'prix_unitaire', val)} /></Table.Td>
                          <Table.Td><Text size="sm" ta="right" fw={600}>{(l.quantite * l.prix_unitaire).toLocaleString()} FCFA</Text></Table.Td>
                          <Table.Td><ActionIcon color="red" variant="subtle" onClick={() => handleRemoveEditLigne(i)}><IconTrash size={14} /></ActionIcon></Table.Td>
                        </Table.Tr>
                      ))}
                    </Table.Tbody>
                  </Table>
              ...
              <Group justify="flex-end" gap="sm">
                <Button variant="light" onClick={() => setEditModalOpen(false)} radius="md">Annuler</Button>
                <Button onClick={handleSaveEditVente} loading={editLoading} radius="md" variant="gradient" gradient={{ from: '#1b365d', to: '#2a4a7a' }}>Enregistrer</Button>
              </Group>
            </Stack>
          )}
        </Modal>
        ...
        <Modal opened={deleteVenteModalOpen} onClose={() => { setDeleteVenteModalOpen(false); setDeleteVenteId(null); setDeleteVenteCode(''); }} title="Confirmation de suppression" size="sm" centered radius="md" styles={{ header: { backgroundColor: '#e03131', padding: '14px 20px' }, title: { color: 'white', fontWeight: 600 }, body: { padding: '20px' } }}>
          <Stack gap="md">
            <Alert color="red" variant="light" radius="md"><Stack gap={4}><Text size="sm" fw={600}>Supprimer la vente "{deleteVenteCode}" ?</Text><Text size="xs">Cette action est irréversible. Le stock sera restauré.</Text></Stack></Alert>
            <Group justify="flex-end" gap="sm"><Button variant="light" onClick={() => { setDeleteVenteModalOpen(false); setDeleteVenteId(null); setDeleteVenteCode(''); }} disabled={loading} radius="md">Annuler</Button><Button color="red" onClick={handleDeleteVente} loading={loading} leftSection={<IconTrash size={16} />} radius="md">Supprimer définitivement</Button></Group>
          </Stack>
        </Modal>
      </Box>
    );
    ...
    <Container size="lg" p={0}>
      <Stack gap="md" p="md">
        <Card withBorder radius="lg" p="md" style={{ background: 'linear-gradient(135deg, #1b365d 0%, #2a4a7a 100%)' }}>
          <Group justify="space-between">
            <Group gap="sm"><ThemeIcon size={42} radius="md" variant="white" color="#1b365d"><IconShoppingBag size={22} /></ThemeIcon><Box><Title order={4} c="white">Nouvelle vente</Title><Text c="gray.3" size="xs">Code : {codeVente}</Text></Box></Group>
            <Group gap={4}><Tooltip label="Liste"><ActionIcon variant="subtle" color="white" onClick={backToList}><IconList size={18} /></ActionIcon></Tooltip><Tooltip label="Reset"><ActionIcon variant="subtle" color="white" onClick={resetForm}><IconRefresh size={18} /></ActionIcon></Tooltip></Group>
          </Group>
        </Card>

        <Card withBorder radius="lg" shadow="sm" p="md">
          <Group justify="space-between" mb="sm"><Title order={5}>👤 Client</Title><SegmentedControl value={venteType} onChange={(val) => { setVenteType(val as VenteType); setPanier([]); }} data={[{ value: 'commande', label: 'Sur mesure' }, { value: 'pret_a_porter', label: 'Prêt-à-porter' }, { value: 'matiere', label: 'Matière' }]} size="xs" color="#1b365d" /></Group>
          <Divider mb="sm" />
          {venteType === 'commande' ? (
            <Stack gap="xs">
        ...
        {(venteType === 'pret_a_porter' || venteType === 'matiere') && (
          <Card withBorder radius="lg" shadow="sm" p="md">
            <Group justify="space-between" mb="sm"><Title order={5}>📦 {venteType === 'pret_a_porter' ? 'Articles disponibles' : 'Matières disponibles'}</Title><Group gap="xs"><TextInput placeholder="Rechercher..." leftSection={<IconSearch size={14} />} value={searchProduitTerm} onChange={(e) => setSearchProduitTerm(e.target.value)} size="xs" radius="md" style={{ width: 200 }} /><Tooltip label="Actualiser"><ActionIcon variant="light" onClick={loadFormData} size="sm" radius="md"><IconRefresh size={14} /></ActionIcon></Tooltip></Group></Group>
              ...
              <Table striped highlightOnHover style={{ fontSize: 12 }}>
                <Table.Thead style={{ backgroundColor: '#1b365d', position: 'sticky', top: 0, zIndex: 1 }}>
                  <Table.Tr>
                    ...
                    <Table.Th style={{ color: 'white', fontSize: 11, padding: '6px 8px', width: 80, textAlign: 'right' }}>Prix vente</Table.Th>
        ...
        {venteType === 'commande' && (
          <Card withBorder radius="lg" shadow="sm" p="md">
            ...
            <Group align="flex-end" gap="xs" mb="sm">
              <TextInput placeholder="Désignation" value={produitCommande} onChange={(e) => setProduitCommande(e.target.value)} size="sm" radius="md" style={{ flex: 2 }} />
              <NumberInput placeholder="Qté" value={quantiteCommande} onChange={(val) => setQuantiteCommande(Number(val) || 1)} min={1} size="sm" radius="md" style={{ width: 70 }} />
              <NumberInput placeholder="Prix unitaire" value={montantCommande} onChange={(val) => setMontantCommande(Number(val) || 0)} min={0} step={500} size="sm" radius="md" style={{ width: 130 }} rightSection={<Text size="xs">FCFA</Text>} />
              <ActionIcon variant="filled" color="#1b365d" size="lg" radius="md" onClick={() => { if (!produitCommande || montantCommande <= 0) { notifications.show({ title: 'Erreur', message: 'Désignation et prix requis', color: 'red' }); return; } setPanier([...panier, { id: `${Date.now()}`, produitId: 0, designation: produitCommande, quantite: quantiteCommande, prixUnitaire: montantCommande, total: quantiteCommande * montantCommande, type_produit: 'article' }]); setProduitCommande(''); setMontantCommande(0); setQuantiteCommande(1); }}><IconPlus size={18} /></ActionIcon>
            </Group>
          </Card>
        )}
        ...
        {venteType === 'commande' && (
          <Card withBorder radius="lg" shadow="sm" p="md" style={{ backgroundColor: '#FFF8E7', borderLeft: '4px solid #1b365d' }}>
            ...
            <Divider mb="sm" />
            <SimpleGrid cols={{ base: 1, sm: 3 }} spacing="md">
              <TextInput label="Date" type="date" placeholder="jj/mm/aaaa" leftSection={<IconCalendar size={14} />} value={dateRendezVous} onChange={(e) => setDateRendezVous(e.target.value)} radius="md" />
              <TextInput label="Heure" type="time" leftSection={<IconClock size={14} />} value={heureRendezVous} onChange={(e) => setHeureRendezVous(e.target.value)} radius="md" />
              <Select label="Type de rendez-vous" value={typeRendezVous} onChange={(v) => setTypeRendezVous(v || 'essayage')} data={[{ value: 'essayage', label: '👗 Essayage' }, { value: 'livraison', label: '🚚 Livraison' }, { value: 'retrait', label: '📦 Retrait' }]} radius="md" />
            </SimpleGrid>
        ...
        <Card withBorder radius="lg" shadow="sm" p="md">
          <SimpleGrid cols={{ base: 1, sm: 2, md: 4 }} spacing="md" mb="md">
            <Box><Text size="xs" c="dimmed">Nb d'articles</Text><Text fw={700} size="lg">{panier.length}</Text></Box>
            <Box><Text size="xs" c="dimmed">Nb de pièces</Text><Text fw={700} size="lg">{panier.reduce((sum, item) => sum + item.quantite, 0)}</Text></Box>
            <Box><Text size="xs" c="dimmed">Montant Total</Text><Text fw={700} size="lg" c="blue">{totalPanier.toLocaleString()} FCFA</Text></Box>
            <Box><Text size="xs" c="dimmed">Date</Text><TextInput type="date" value={dateCommande} onChange={(e) => setDateCommande(e.target.value)} size="xs" radius="md" /></Box>
          </SimpleGrid>
          <Divider mb="md" />
          <Textarea label="Observation" placeholder="Notes..." value={observation} onChange={(e) => setObservation(e.target.value)} rows={2} size="sm" radius="md" mb="md" />
          {venteType === 'commande' ? (
            <Stack gap="sm">
              <Button onClick={handleGenererFacture} size="md" radius="md" variant="gradient" gradient={{ from: '#1b365d', to: '#2a4a7a' }} leftSection={<IconFileInvoice size={18} />} fullWidth disabled={panier.length === 0}>Générer la facture</Button>
              <Button onClick={handleSubmitVente} loading={loading} size="md" radius="md" variant="gradient" gradient={{ from: 'green', to: 'teal' }} leftSection={<IconReceipt size={18} />} fullWidth disabled={panier.length === 0}>Enregistrer la commande</Button>
            </Stack>
          ) : (
            <Button onClick={handleSubmitVente} loading={loading} size="md" radius="md" variant="gradient" gradient={{ from: 'green', to: 'teal' }} leftSection={<IconReceipt size={18} />} fullWidth disabled={panier.length === 0}>Finaliser la vente</Button>
          )}
        </Card>
          ...
          <Stack gap="md">
            {selectedArticle && (
              <Paper p="sm" withBorder radius="md" bg="gray.0">
                ...
                <Group justify="space-between" mt={4}><Text size="xs" c="dimmed">Stock : {selectedArticle.quantite_stock}</Text><Text size="xs" c="dimmed">Prix : {selectedArticle.prix_vente.toLocaleString()} FCFA</Text></Group>
              </Paper>
            )}
            <NumberInput label="Quantité à ajouter" value={quantiteCmd} onChange={(val) => setQuantiteCmd(typeof val === 'number' ? Math.max(1, val) : 1)} min={1} max={selectedArticle?.quantite_stock || 1} size="sm" radius="md" autoFocus />
            <Group justify="flex-end" gap="xs"><Button variant="subtle" size="xs" onClick={() => { setShowQuantiteModal(false); setSelectedArticle(null); }}>Annuler</Button><Button size="xs" onClick={() => { setShowQuantiteModal(false); handleAjouterAuPanier(); }}>Ajouter au panier</Button></Group>
          </Stack>
        </Modal>
      </Stack>
    </Container>
